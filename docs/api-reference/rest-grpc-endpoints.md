...
:::note
These interfaces are available on both the **`qorechain-vladi`** mainnet (live since 7 June 2026 on chain version **v3.1.95**) and the **`qorechain-diana`** testnet. The base URLs below assume a locally running node; the public hosted endpoints (`rpc/api/evm/svm.qore.host` and their `-testnet` variants) are listed in [Networks](/appendix/networks#public-endpoints).
:::
