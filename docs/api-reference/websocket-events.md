...
:::note
Both WebSocket interfaces are available on the **`qorechain-vladi`** mainnet (live on chain version **v3.1.82**) and the **`qorechain-diana`** testnet. The local endpoints below assume a node you run yourself; substitute your provider's mainnet or testnet host for remote access.
:::
