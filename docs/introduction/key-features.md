...
The following table lists every major feature in QoreChain, organized by the release phase in which it was introduced. The current chain version is **v3.1.77**, with mainnet (`qorechain-vladi`, EVM chain ID 9801) live since 7 June 2026 and a parallel testnet (`qorechain-diana`, EVM chain ID 9800).

| Feature                    | Introduced in       | Description                                                                                                                                                                                                                                                                                                                                                                                     |
| -------------------------- | ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PQC Hybrid Signatures (required by default) | v1.1.0 (Security)   | Dual signatures on every cosmos-path transaction: a classical secp256k1 (ECDSA) signature paired with ML-DSA-87 (Dilithium-5). As of v3.1.71 the network default is **required** (`hybrid_signature_mode = required`, `allow_classical_fallback = false`) — classical-only cosmos txs are rejected; only genesis gentxs and PQC key registration/migration txs are exempt. EVM transactions use a separate `eth_secp256k1` path and are unaffected. Three governance-controlled enforcement modes (disabled / optional / required) remain available. Seamless wallet onboarding via TX extension auto-registration.                                                                              |
| SHAKE-256 Default Hash     | v1.1.0 (Security)   | SHA-3 family extendable-output function (XOF). As of v3.1.73, SHAKE-256 (via the `qorehash` package) is the **default application hash**, completing the PQC baseline (Dilithium-5 + ML-KEM-1024 + SHAKE-256). Provides variable-length hashing, fixed 32-byte output, Merkle internal node concatenation, and domain-separated hashing — all in pure Go with no FFI dependency.                                                                                                                 |
