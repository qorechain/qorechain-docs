...
:::caution 新規参加ノードにはv3.1.94以降が必須
ジェネシスから同期する、またはアーカイブ/スナップショットからリプレイするノードは**v3.1.94以降**である必要があります。理由は2つ重なっています——v3.1.92では、修正済みとなったガス計測のバグにより、リプレイ中にトランザクションを含む最初のブロックで停止していましたが、それに加えてメインネットはその後v3.1.94のガバナンスアップグレード(発行上限のハードキャップ、ブロック高2,122,074で適用)を経ています——このアップグレードのハンドラーを持たないノードは、同じブロック高をリプレイしようとして再び停止します。v3.1.95が現在推奨されるバージョンです(コンセンサスを破壊しない、継続的なセキュリティアップデート)。`minCompatible`は`3.1.94`です。マニフェストはまずテストネットで、猶予期間を置いてからメインネットで昇格されるため意図的に遅れて反映されることがあり、これまでもこの下限より遅れていたことがあります——`binary.url`を信頼する前にマニフェストの`"version"`フィールドを確認し、遅れている場合は[qorechain-core GitHub リリース](https://github.com/qorechain/qorechain-core/releases)を使うか、ソースからビルドしてください。
:::
...
Docker Composeによるノードのみのデプロイです。まだ公開されている`qorechaind`イメージは存在しないため、リポジトリの`Dockerfile`から自分でビルドし、稼働中のチェーンバージョン(メインネットでは**v3.1.95**)にタグを合わせ、チェーンデータ用に永続ボリュームをマウントしてください。
...
以下の手順では、このファイルをバイナリ、ジェネシス、ピアの値の情報源として使用してください——`jq -r .minCompatible latest.json`を確認しますが、そのフィールドが遅れていても上記の**v3.1.94の下限**が適用されることに変わりはない点に注意してください。
...
:::note
スナップショットは頻繁に変化する**ブロック高が刻まれたファイル名**で公開されます——最新のスナップショットとそのSHA-256チェックサムについては[download.qore.host](https://download.qore.host)を確認し、展開前に必ず検証してください。上記の**v3.1.94という最低要件**は、スナップショットからのリプレイにも適用される点に注意してください。
:::
...
1. **チェーンバージョンを固定する。** 稼働中のタグ(メインネットでは**v3.1.95**)を実行し、協調アップグレードのため公式リリースを追跡してください。
