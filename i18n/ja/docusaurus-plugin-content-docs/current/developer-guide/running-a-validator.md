...
バリデータは、ML-DSA-87 アルゴリズムを用いた**耐量子暗号（PQC）公開鍵**を任意で登録できます。これにより、バリデータのアイデンティティに量子耐性のセキュリティが提供され、ハイブリッド署名にも利用できます。
...
:::tip
**推奨事項:** PQC 鍵の登録は任意ですが、メインネットで運用するバリデータには強く推奨されます。量子コンピューティングの脅威に対する将来を見据えた防御となります。
:::
