...
:::note
このガイドは、2026 年 6 月 7 日から稼働し、チェーンバージョン **v3.1.85** を実行している **`qorechain-vladi`** メインネット(EVM チェーン ID **9801**)を対象としています。本番稼働前のセットアップのリハーサルには、**`qorechain-diana`** テストネット(EVM チェーン ID **9800**)の利用を推奨します。対象ネットワークに応じて適切な `--chain-id` に置き換えてください。
:::
