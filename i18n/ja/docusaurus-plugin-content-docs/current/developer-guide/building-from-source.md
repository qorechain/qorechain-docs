...
`init` コマンドは、エラーなく `~/.qorechaind/` に genesis ファイルとノード設定を作成するはずです。上記の例は **`qorechain-diana`** テストネットに対して初期化しています。メインネットの場合は、チェーンバージョン **v3.1.95** を実行する稼働中ネットワークである `--chain-id qorechain-vladi` に置き換えてください。
