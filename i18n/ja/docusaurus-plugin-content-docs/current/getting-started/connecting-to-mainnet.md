...
:::note
このページは、2026 年 6 月 7 日 23:59 UTC からチェーンバージョン **v3.1.80**（Cosmos SDK v0.53 上）で稼働している **`qorechain-vladi`** メインネット（EVM チェーン ID **9801**、16 進数 `0x2649`）を対象としています。**`qorechain-diana`** テストネット（EVM チェーン ID **9800**）については、[テストネットへの接続](/getting-started/connecting-to-testnet) を参照し、本番稼働前にそこでセットアップをリハーサルしてください。
:::
