...
:::note
このページは、Cosmos SDK v0.53上でチェーンバージョン**v3.1.95**を稼働し、**2026年6月7日 23:59 UTC**から稼働している**`qorechain-vladi`**メインネット(EVMチェーンID **9801**、16進数`0x2649`)を対象としています。**`qorechain-diana`**テストネット(EVMチェーンID **9800**)については、[テストネットへの接続](/getting-started/connecting-to-testnet)を参照し、本番稼働前にそちらでセットアップをリハーサルしてください。
:::
...
:::caution ノードを最新に保つ — 新規同期にはv3.1.94以降が必須
フルノードはネットワークの現行チェーンバージョンに追随する必要があります。マニフェストが指すバイナリを常にインストールし、古いバージョンに固定しないでください。ジェネシスから新規参加する、または停止から復旧するノードの下限は**v3.1.94以降**であり、これは二つの独立した、積み重なった理由によるものです。v3.1.92では、トランザクションを含む最初のブロックでリプレイが停止してしまうガス計測バグが修正されました。その後、メインネットはv3.1.94のガバナンスアップグレード(発行上限のハードキャップを、高さ2,122,074で適用)を通過しており、そのアップグレードのハンドラを持たないノードは、同じ高さをリプレイしようとして再び停止してしまいます。v3.1.95が現在の推奨バージョンです(継続的なセキュリティ更新であり、`minCompatible`は`3.1.94`です)。`binary.url`を信頼する前に、必ずマニフェストの`"version"`フィールドを現在のバージョン下限と照合してください。マニフェストは意図的に段階的に展開されます(まずテストネット、そして一定のソーク期間を経てメインネットという順序)ため、これまでにもバージョン下限に遅れたことがあります。
:::
