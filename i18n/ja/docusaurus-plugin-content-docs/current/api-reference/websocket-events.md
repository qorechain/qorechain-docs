...
:::note
どちらのWebSocketインターフェースも、**`qorechain-vladi`** メインネット(チェーンバージョン **v3.1.95** で稼働中)と **`qorechain-diana`** テストネットの両方で利用できます。以下のローカルエンドポイントは、自分で運用するノードを前提としています。リモートアクセスの場合は、ご利用のプロバイダーのメインネットまたはテストネットのホストに置き換えてください。
:::
