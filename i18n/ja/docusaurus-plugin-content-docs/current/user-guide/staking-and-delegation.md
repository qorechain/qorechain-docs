...
:::note
以下のコマンドは **`qorechain-diana`** テストネット（EVM チェーン ID **9800**）を使用しています。メインネット（**`qorechain-vladi`**、EVM チェーン ID **9801**）は 2026 年 6 月 7 日以降稼働しており、チェーンバージョン **v3.1.77** を実行しています。メインネットでステーキングを行う場合は、**メインネットへの接続** ページにあるメインネットのチェーン ID とエンドポイントに置き換えてください。
:::
