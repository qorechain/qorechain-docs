...
:::note
以下のコマンドは**`qorechain-diana`**テストネット(EVMチェーンID **9800**)を使用しています。メインネット(**`qorechain-vladi`**、EVMチェーンID **9801**)はチェーンバージョン**v3.1.97**を実行して2026年6月7日から稼働しています — メインネットでステーキングする際は、**Connecting to Mainnet**ページに記載のメインネットのチェーンIDとエンドポイントに置き換えてください。
:::
