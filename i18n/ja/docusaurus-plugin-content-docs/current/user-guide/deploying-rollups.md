...
:::note
以下のコマンドは **`qorechain-diana`** テストネット(EVMチェーンID **9800**)を使用しています。メインネット(**`qorechain-vladi`**、EVMチェーンID **9801**)は2026年6月7日からチェーンバージョン **v3.1.97** で稼働しています — メインネットにデプロイする際は、**Connecting to Mainnet** ページに記載のメインネット用チェーンIDとエンドポイントに置き換えてください。
:::
