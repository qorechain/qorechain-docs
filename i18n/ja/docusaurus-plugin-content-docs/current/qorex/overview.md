...
| iOS | 1.0.9 |
| Firefox | 0.2.6 |
| Safari（macOS） | 1.6（拡張機能 0.2.6 を内蔵） |
| Chrome | 8月下旬時点で長期の審査待ちの状態が続いています — 現在のバージョンはここに記載の数字を鵜呑みにせず、[Chrome Web Store の掲載ページ](https://chromewebstore.google.com/detail/qorex/cflpnggbnnifibblifgbeobogdpfjpcg)で直接ご確認ください |

1.0.8 / 0.2.6 では、21日間のアンボンディング待機なしで **ステーキング済み QOR をバリデーター間で移動する**（リデリゲート）機能が追加されました。詳細は[バリデーター間でのステーク移動](/qorex/portfolio-and-staking#move-stake)をご覧ください。**1.0.9**（モバイルのみ）では、資産のあるアカウントが「保有アカウント」から一時的に消えてしまう不具合を修正しました — [トラブルシューティング](/qorex/troubleshooting)を参照 — また、金額の表示方法と入力方法を改善しました。
