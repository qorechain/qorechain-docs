...
ライトノードは**報酬を獲得する**と同時に、それを獲得し続けるために**健全な状態を保つ必要があります**。このページでは、3%のライトノード報酬シェア、委任ステーキングと自動複利運用の仕組み、そしてノードの監視方法について説明します。
...
このシェアの対象となるには、自己申告ではなくオンチェーンで確認される3つの条件をノードが満たす必要があります。有効な `lightnode_operator` ライセンス、**最低1,000 QORの委任**（バリデーターごとではなく、委任しているすべてのバリデーターにまたがる合計で数えます）、そして**1 QOR**のオンチェーン登録手数料です。参加はネットワーク全体で**10,000ライトノード**を上限としています。登録とライセンスの仕組み、および報酬プログラム登録の現在のステータスについては、[登録とライセンス](/light-node/registration-and-licensing)を参照してください。

登録と委任が完了した後は、対象資格を維持するには稼働し続けることが重要です。ノードは最低**80%の稼働率**を維持し、およそ**1,000ブロック（約39分）間隔**でハートビート稼働証明を提出し続ける必要があります。ハートビートを1回逃した場合でも、非アクティブと判定されるまでには**約100ブロック（約4分）の猶予期間**があります。非アクティブと判定されたノードは、再び稼働を証明するまでシェアの獲得を停止します。

*報酬の対象条件: 有効なオンチェーンライセンスと最低委任ステーク額を保持し、登録を済ませたうえで、シェアの獲得を継続させる稼働率としきい値を維持できるよう、ハートビートによる稼働証明を続けること。*
...
これは keygen → sign → verify → 改ざん検出（5つのチェック）を実行し、いずれかが失敗すると非ゼロで終了します。これは、ノードの問題を診断する際に、ポスト量子署名スタックの問題を除外する最も速い方法です。完全なセルフテストの内訳については[SX エディション](/light-node/sx-edition)を参照してください。
