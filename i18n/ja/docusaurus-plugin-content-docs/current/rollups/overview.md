...
:::caution
RDK およびロールアップ決済レイヤーは、現在も活発に進化している機能です。本セクション全体で説明される決済モード、証明システム、プリセット、機能ごとの成熟度は、変更される可能性のある設計意図として扱い、メインネット（**`qorechain-vladi`**、EVM チェーン ID **9801**、チェーンバージョン **v3.1.82**）を対象とする前に **`qorechain-diana`** テストネット上で任意のデプロイを検証してください。
:::
