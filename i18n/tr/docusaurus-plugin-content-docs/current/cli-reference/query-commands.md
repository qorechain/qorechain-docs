...
:::note
Sorgular, `--node` parametresinin işaret ettiği düğüme karşı çalışır. Canlı veriler için bir **`qorechain-vladi`** mainnet RPC uç noktası (zincir sürümü **v3.1.97**) veya test için bir **`qorechain-diana`** testnet uç noktası kullanın. Varsayılan `tcp://localhost:26657` kendi çalıştırdığınız bir düğümü hedefler.
:::
