---
slug: /developer-guide/svm-development
title: SVM Geliştirme
sidebar_label: SVM Geliştirme
sidebar_position: 4
---

# SVM Geliştirme

QoreChain, bir **Solana Sanal Makinesi (SVM)** yürütme ortamı içerir; bu, geliştiricilerin tanıdık Solana araçlarını kullanarak SBF/BPF programlarını dağıtmasına ve yürütmesine olanak tanır. SVM modülü, **8899 portunda** Solana uyumlu bir JSON-RPC arayüzü sunar; bunu `qorechaind start` otomatik olarak başlatır (aşağıdaki [JSON-RPC Sunucusu](#json-rpc-server) bölümüne bakın).

:::note
Aşağıdaki komutlar, 7 Haziran 2026'dan beri canlı olan ve **v3.1.80** zincir sürümünü çalıştıran **`qorechain-vladi`** ana ağını kullanır. Test ağı için `--chain-id qorechain-diana` ile değiştirin.
:::

---

## Genel Bakış

`x/svm` modülü şunları sağlar:

* SBF/BPF program dağıtımı ve yürütmesi
* Veri hesabı oluşturma ve yönetimi
* Solana uyumlu bir JSON-RPC uç noktası
* QoreChain ve Solana adres biçimleri arasında çift yönlü adres eşlemesi
* Hesaplama bütçesi ölçümü ve kira tabanlı depolama ekonomisi

---

## JSON-RPC Sunucusu {#json-rpc-server}

Solana uyumlu JSON-RPC sunucusu **`qorechaind start` tarafından başlatılır** ve **varsayılan olarak etkindir**. `app.toml` içindeki bir `[svm-rpc]` bölümü aracılığıyla yapılandırılır:

```toml
[svm-rpc]
# Enable the Solana-compatible JSON-RPC server
enable = true
# Address the server listens on
address = "127.0.0.1:8899"
```

Varsayılanlar `enable = true` ve `address = "127.0.0.1:8899"`'dur; bu nedenle yeni başlatılan bir düğüm, 8899 portunda Solana JSON-RPC arayüzünü zaten sunar — `@solana/web3.js` ek bir kuruluma gerek kalmadan `http://127.0.0.1:8899` adresinden bağlanır. `getVersion`, `1.18.0-qorechain` raporlar ve `getBalance` / `getAccountInfo`, canlı zincir üzerindeki SVM hesaplarını döndürür.

| Özellik       | Değer                     |
| ------------- | ------------------------- |
| Varsayılan URL| `http://127.0.0.1:8899`   |
| Etkin         | Evet, varsayılan olarak   |
| Başlatan      | `qorechaind start`        |
| Uyumluluk     | Solana JSON-RPC (alt küme)|
| `getVersion`  | `1.18.0-qorechain`        |

### Desteklenen Yöntemler

| Yöntem                              | Açıklama                                   |
| ----------------------------------- | ----------------------------------------- |
| `getAccountInfo`                    | Hesap verilerini ve lamport bakiyesini alma |
| `getBalance`                        | Hesap bakiyesini lamport cinsinden alma   |
| `getSlot`                           | Geçerli slot numarası                      |
| `getMinimumBalanceForRentExemption` | Belirli bir veri boyutu için minimum bakiye |
| `getVersion`                        | SVM çalışma zamanı sürüm bilgisi          |
| `getHealth`                         | SVM uç noktası için sağlık kontrolü       |

---

## Programları Dağıtma ve Onlarla Etkileşim Kurma

:::info
**Modern SBF yürütmesi.** SVM yürütme motoru **solana-sbpf 0.21.1** üzerine modernize edildi; bu nedenle geçerli Solana araç zincirinden (**platform-tools v1.53 / agave 4.x**) yeni derlenen SBF programları QoreChain üzerinde hem **dağıtılır hem de çalışır** — yürütme yalnızca dağıtımla sınırlı değil, tam olarak desteklenir. Hem `cargo build-sbf --arch v0` hem de `--arch v3` ile derlenen programlar desteklenir.
:::

1. **Bir SBF Programı Dağıtma** — Solana programınızı geçerli platform-tools (v1.53 / agave 4.x) ile bir SBF paylaşımlı nesnesine derleyin, ardından QoreChain'e dağıtın:

   ```bash
   # Build with the current Solana toolchain (--arch v0 or --arch v3)
   cargo build-sbf --arch v3

   # Deploy the compiled program
   qorechaind tx svm deploy-program ./my_program.so \
     --from mykey \
     --gas auto \
     --gas-adjustment 1.3 \
     -y
   ```

   İşlem yanıtı, **program kimliğini** base58 biçiminde içerir.

2. **Bir Talimat Yürütme** — Zincir üzerindeki bir BPF programını talimat verisiyle çağırın:

   ```bash
   # Execute instruction
   qorechaind tx svm execute <program-id-base58> <data-hex> \
     --from mykey \
     --gas auto \
     -y
   ```

   | Parametre           | Biçim             | Açıklama                       |
   | ------------------- | ----------------- | ------------------------------ |
   | `program-id-base58` | Base58 dizesi     | Dağıtılan programın adresi     |
   | `data-hex`          | Onaltılık kodlu bayt | Serileştirilmiş talimat verisi |

3. **Bir Veri Hesabı Oluşturma** — Programlar genellikle durum saklamak için hesaplara ihtiyaç duyar. Belirli bir boyut ve sahip ile bir tane oluşturun:

   ```bash
   # Create data account
   qorechaind tx svm create-account <owner-base58> <space> <lamports> \
     --from mykey \
     --gas auto \
     -y
   ```

   | Parametre      | Açıklama                                           |
   | -------------- | -------------------------------------------------- |
   | `owner-base58` | Bu hesabın sahibi olan program (base58)            |
   | `space`        | Veri alanının bayt cinsinden boyutu                |
   | `lamports`     | Başlangıç bakiyesi (kira muafiyeti minimumunu karşılamalıdır) |

   Belirli bir boyut için minimum kira muaf bakiyesini sorgulayın:

   ```bash
   # RPC: getMinimumBalanceForRentExemption
   curl -X POST http://localhost:8899 \
     -H "Content-Type: application/json" \
     -d '{
       "jsonrpc": "2.0",
       "id": 1,
       "method": "getMinimumBalanceForRentExemption",
       "params": [1024]
     }'
   ```

4. **@solana/web3.js Kullanımı** — Solana JavaScript SDK'sı, QoreChain SVM uç noktasıyla doğrudan çalışır:

   ```javascript
   import { Connection, PublicKey } from "@solana/web3.js";

   const connection = new Connection("http://127.0.0.1:8899");

   // Check health
   const health = await connection.getHealth();
   console.log("SVM health:", health);

   // Get slot
   const slot = await connection.getSlot();
   console.log("Current slot:", slot);

   // Get account info
   const pubkey = new PublicKey("YourBase58ProgramId...");
   const accountInfo = await connection.getAccountInfo(pubkey);
   console.log("Account data:", accountInfo);

   // Get balance
   const balance = await connection.getBalance(pubkey);
   console.log("Balance (lamports):", balance);
   ```

---

## Adres Eşlemesi

QoreChain, yerel Bech32 adresleri (`qor1...`) ile Solana tarzı base58 adresleri arasında **çift yönlü bir adres eşlemesi** sürdürür:

| Yön           | Örnek                                                       |
| ------------- | ---------------------------------------------------------- |
| Yerelden SVM'ye | `qor1abc...xyz`, deterministik bir base58 adresine eşlenir |
| SVM'den Yerele | Base58 program adresleri, `qor1...` eşdeğerlerine geri eşlenir |

Eşleme deterministiktir ve `x/svm` modülü tarafından yönetilir. Her iki gösterim de aynı temel hesabı ifade eder.

---

## Kira Modeli

SVM modülü, durum şişmesini önlemek için **kira tabanlı bir depolama modeli** kullanır:

| Parametre                    | Değer      |
| ---------------------------- | ---------- |
| Bayt başına yıllık lamport   | `3,480`    |
| Kira muafiyeti çarpanı       | `2.0`      |
| Toplama sıklığı              | Her epok   |

* Lamport cinsinden bakiyesi `2 * (data_size * 3480 / seconds_per_year)` değerinin **üzerinde** olan hesaplar **kiradan muaftır** ve hiçbir zaman ücretlendirilmez.
* Kira muafiyeti eşiğinin **altındaki** hesaplar her epok kira öder. Bakiye sıfıra ulaşırsa hesap temizlenir.

:::info
**En iyi uygulama:** Beklenmedik hesap silinmesini önlemek için veri hesaplarını her zaman kira muafiyeti minimumunun üzerinde fonlayın.
:::

---

## Hesaplama Bütçesi

Her talimat yürütmesi, hesaplama birimleriyle ölçülür:

| Parametre                                | Değer       |
| ---------------------------------------- | ----------- |
| Talimat başına maksimum hesaplama birimi | `1,400,000` |
| Maksimum CPI (programlar arası çağrı) derinliği | `4`  |
| Maksimum program boyutu                  | `10 MB`     |
| Maksimum hesap veri boyutu               | `10 MB`     |

Hesaplama bütçesini aşan programlar durdurulur ve işlem geri alınır.

---

## Parametre Özeti

| Parametre                   | Değer        |
| --------------------------- | ------------ |
| `max_program_size`          | 10 MB        |
| `max_account_data_size`     | 10 MB        |
| `compute_budget_max`        | 1,400,000 CU |
| `max_cpi_depth`             | 4            |
| `lamports_per_byte_year`    | 3,480        |
| `rent_exemption_multiplier` | 2.0          |
| JSON-RPC portu              | 8899         |

---

## VM'ler Arası Birlikte Çalışabilirlik

SVM programları, **asenkron** VM'ler arası mesaj yolu aracılığıyla EVM ve CosmWasm sözleşmeleriyle iletişim kurabilir:

```bash
# Cross-VM call example
qorechaind tx crossvm call \
  --source-vm svm \
  --target-vm evm \
  --target-contract 0x1234...abcd \
  --payload '...' \
  --from mykey \
  -y
```

Mesajlar sıraya alınır ve EndBlocker tarafından işlenir. Mesaj yaşam döngüsü ve zaman aşımı davranışı hakkında ayrıntılar için [VM'ler Arası Birlikte Çalışabilirlik](/developer-guide/cross-vm-interoperability) bölümüne bakın.

---

## Sonraki Adımlar

* [VM'ler Arası Birlikte Çalışabilirlik](/developer-guide/cross-vm-interoperability) — SVM, EVM ve CosmWasm arasında iletişim
* [EVM Geliştirme](/developer-guide/evm-development) — QoreChain üzerinde Solidity akıllı sözleşmeleri
* [CosmWasm Geliştirme](/developer-guide/cosmwasm-development) — Rust tabanlı WebAssembly sözleşmeleri
