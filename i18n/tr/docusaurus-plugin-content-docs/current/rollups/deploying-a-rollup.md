...
:::note
Aşağıdaki komutlar **`qorechain-diana`** test ağını hedef alır. Mainnet (**`qorechain-vladi`**, EVM zincir ID **9801**) 7 Haziran 2026'dan beri **v3.1.97** zincir sürümünü çalıştırarak canlıdır — mainnet'te dağıtım yaparken mainnet zincir ID'sini ve uç noktalarını kullanın. Her dağıtımı önce test ağında doğrulayın.
:::
