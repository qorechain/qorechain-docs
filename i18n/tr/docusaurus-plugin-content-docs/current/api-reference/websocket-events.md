...
:::note
Her iki WebSocket arayüzü de **`qorechain-vladi`** mainnet'inde (zincir sürümü **v3.1.97** ile canlı) ve **`qorechain-diana`** testnet'inde kullanılabilir. Aşağıdaki yerel uç noktalar, kendi işlettiğiniz bir düğümü varsayar; uzaktan erişim için sağlayıcınızın mainnet veya testnet ana bilgisayarını kullanın.
:::
