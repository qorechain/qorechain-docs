...
:::note
Her iki WebSocket arayüzü de **`qorechain-vladi`** ana ağında (**v3.1.82** zincir sürümünde canlı) ve **`qorechain-diana`** test ağında kullanılabilir. Aşağıdaki yerel uç noktalar kendi çalıştırdığınız bir düğümü varsayar; uzaktan erişim için sağlayıcınızın ana ağ veya test ağı ana makinesini kullanın.
:::
