...
:::note
Die folgenden Befehle verwenden das **`qorechain-diana`**-Testnet (EVM-Chain-ID **9800**). Das Mainnet (**`qorechain-vladi`**, EVM-Chain-ID **9801**) ist seit dem 7. Juni 2026 aktiv — ersetzen Sie beim Transagieren auf dem Mainnet die Chain-ID und die Endpunkte des Mainnets von der Seite **Verbindung zum Mainnet**.
:::
...
Dies sendet **1 QOR** (1.000.000 uqor) an die Empfängeradresse und zahlt dabei eine Gebühr von 500 uqor.

:::caution Cosmos-Transfers erfordern eine hybride PQC-Signatur
Auf dem Cosmos-Pfad ist der Netzwerkstandard `hybrid_signature_mode = required` (aktuelle Chain-Version **v3.1.95**). Ein einfaches klassisches `tx bank send` wird **abgelehnt** — jede Transaktion auf dem Cosmos-Pfad muss neben der secp256k1-Signatur eine ML-DSA-87-Signatur (Dilithium-5) tragen. Erzeugen Sie einen Dilithium-5-Schlüssel mit `qorechaind tx pqc gen-key` und hängen Sie anschließend die hybride Co-Signatur mit `qorechaind tx pqc cosign` an (oder erstellen Sie die Transaktion mit `buildHybridTx` des QoreChain-SDK unter Verwendung von `includePqcPublicKey`, damit sich der Schlüssel bei der ersten Verwendung automatisch registriert). Um die hybride Signatur außerhalb der CLI zu erzeugen, leisten die quelloffene Bibliothek [**qorechain-pqc**](/developer-guide/post-quantum-signing) (`hybridSignBytes`) und das QoreChain-SDK im Code das Gleiche. Den vollständigen hybriden Ablauf finden Sie unter [Wallet-Einrichtung](/getting-started/wallet-setup).
:::
...
Die EVM-Ausführungsumgebung von QoreChain stellt an Port `8545` eine standardmäßige Ethereum-JSON-RPC-Schnittstelle bereit.

:::note
EVM-Transaktionen sind von der hybriden PQC-Anforderung des Cosmos-Pfads **nicht betroffen**. Sie verwenden einen separaten `eth_secp256k1`-Ante-Pfad, sodass standardmäßiges Ethereum-Signieren (MetaMask, ethers.js usw.) ohne PQC-Erweiterung funktioniert.
:::
...
Die SVM-Ausführungsumgebung von QoreChain stellt an Port `8899` eine Solana-kompatible RPC-Schnittstelle bereit.
