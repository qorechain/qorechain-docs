...
Alle REST-Endpunkte liefern JSON zurück. gRPC-Endpunkte verwenden Protocol Buffers und können mit jedem beliebigen gRPC-Client genutzt werden. Die RPC-Schnittstelle stellt Abfragen auf Konsensebene sowie das Broadcasten von Transaktionen bereit.

:::note
Diese Schnittstellen sind sowohl im **`qorechain-vladi`**-Mainnet (live seit dem 7. Juni 2026 in Chain-Version **v3.1.80**) als auch im **`qorechain-diana`**-Testnet verfügbar. Die untenstehenden Basis-URLs gehen von einem lokal laufenden Node aus; ersetzen Sie für den Remote-Zugriff den Mainnet- oder Testnet-Host Ihres Anbieters.
:::
...
| GET     | `/ai/v1/fee-estimate`              | AI-gestützte Gas-Gebührenschätzung für eine Transaktion |
...
Ab Chain-Version **v3.1.77** wird der schreibgeschützte Zustand des Bridge-Moduls über REST via grpc-gateway unter dem Präfix `/qorechain/bridge/v1/...` bereitgestellt (zuvor nur per gRPC). Diese Endpunkte liefern echtes On-Chain-JSON über HTTP für Explorer und Light-Node-Telemetrie. Die Bridge-`config` meldet z. B. `min_validators=10` und `threshold=7`.
...
| GET     | `/qorechain/bridge/v1/chains/{chain_id}`   | Details zu einer bestimmten gebridgten Chain  |
...
| GET     | `/bridge/v1/chains/{id}`            | Details zu einer bestimmten gebridgten Chain        |
...
| GET     | `/bridge/v1/limits/{chain}`         | Ratenlimits und Schwellenwerte für eine gebridgte Chain |
...
Ab Chain-Version **v3.1.80** wird der vollständige Query-Dienst des Multilayer-Moduls über REST via grpc-gateway unter dem Präfix `/qorechain/multilayer/v1/...` bereitgestellt (zuvor nur per gRPC), einschließlich zweier **State-Anchor-Leseabfragen**: `anchor/{layer_id}` gibt den neuesten Settlement-Anchor für einen Layer zurück, und `anchors/{layer_id}` gibt dessen Anchor-Historie zurück. Jeder Anchor trägt eine **ML-DSA-87 (Dilithium-5)**-Signatur über seine kanonischen Felder, sodass ein Client einen Anchor abrufen und ihn unabhängig verifizieren kann — die On-Chain-Grundlage für die [Settlement-Belege](/rollups/settlement-receipts) des Rollup Development Kit.
...
| GET     | `/qorechain/multilayer/v1/routing-stats`        | Statistiken zum Transaktions-Routing über Layer hinweg |

Ein `StateAnchorView` enthält die Felder `layer_id`, `layer_height`, `state_root`, `validator_set_hash`, `main_chain_height`, `anchored_at`, `pqc_aggregate_signature`, `transaction_count` und `compressed_state_proof`. Die signierte kanonische Nachricht lautet `layer_id || layer_height || state_root || validator_set_hash` und wird gegen den registrierten PQC-Schlüssel des Layer-Erstellers verifiziert.
...
| GET     | `/multilayer/v1/routing-stats` | Statistiken zum Transaktions-Routing über Layer hinweg |
...
Ab Chain-Version **v3.1.77** wird der schreibgeschützte Zustand des Burn-Moduls über REST via grpc-gateway unter dem Präfix `/qorechain/burn/v1/...` bereitgestellt (zuvor nur per gRPC). Diese Endpunkte liefern echtes On-Chain-JSON über HTTP für Explorer und Light-Node-Telemetrie. Die Burn-`stats` enthalten z. B. `gas_burn_rate=0.30`.
...
| Methode | Endpunkt                             | Beschreibung                                  |
| ------- | ------------------------------------ | --------------------------------------------- |
| GET     | `/gasabstraction/v1/accepted-tokens` | Listet die für die Gas-Zahlung akzeptierten Token auf |
| GET     | `/gasabstraction/v1/params`          | Aktuelle Parameter des Gas-Abstraction-Moduls |

## gRPC-Reflection

Die gRPC-Server-Reflection ist standardmäßig aktiviert, sodass Tools wie `grpcurl` die verfügbaren Dienste ermitteln können:
...
Alle REST- und gRPC-Endpunkte sind standardmäßig nicht authentifiziert. Platzieren Sie für Produktionsbereitstellungen einen Reverse-Proxy (z. B. Nginx oder Caddy) vor dem Node, um die TLS-Terminierung und Zugriffskontrolle zu übernehmen.
