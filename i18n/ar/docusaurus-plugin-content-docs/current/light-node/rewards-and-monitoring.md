...
لتكون مؤهلًا لهذه الحصة، تحتاج العقدة إلى ثلاثة أمور، يتم التحقق منها على السلسلة لا بالتصريح الذاتي: ترخيص `lightnode_operator` نشط، وحد أدنى قدره **1,000 QOR مفوَّضة** — تُحسب كإجمالي عبر جميع المُدققين الذين تفوِّض لهم، وليس لكل مُدقق على حدة — ورسم تسجيل على السلسلة قدره **1 QOR**. كما أن المشاركة محدودة على مستوى الشبكة بسقف **10,000 عقدة خفيفة**. راجع [التسجيل والترخيص](/light-node/registration-and-licensing) لمعرفة كيفية عمل التسجيل والترخيص، بما في ذلك الحالة الراهنة للتسجيل في برنامج المكافآت.

بمجرد التسجيل والتفويض، يصبح الحفاظ على الأهلية مسألة بقاء نشطًا. تحتاج العقدة إلى نسبة تشغيل لا تقل عن **80%**، ويجب أن تستمر في إرسال إثباتات استمرارية النشاط بنبضات القلب على فترة تبلغ تقريبًا **1,000 كتلة (~39 دقيقة)**.

**نافذة الإرسال ضيقة من الطرفين، لا من جانب التأخر فقط.** لا تُقبَل نبضة القلب إلا في المدى الممتد تقريبًا بين **آخر نبضة قلب مقبولة + 1,000 كتلة و+ 1,100 كتلة** (نحو 4 دقائق، مرة كل ~39 دقيقة) — الإرسال المبكر جدًا يُرفض تمامًا كالإرسال المتأخر جدًا.

**تفويت النافذة يكلِّف نسبة التشغيل، لا التسجيل.** العقدة التي تفوِّت تُعلَّم غير نشطة وتتوقف عن كسب الحصة، لكن أول نبضة قلب ناجحة تالية تعيد تفعيلها — دون حاجة لإعادة تسجيل. لاحظ أيضًا أن العدَّاد الداخلي الخاص بالبرنامج الخلفي نحو نبضة القلب التالية يستمر في التقدم حتى لو فشلت محاولة الإرسال، ويُعاد ضبطه عند إعادة التشغيل، لذا قد تُعلَّم العقدة غير نشطة دون أي خطأ من المشغِّل — تحقق من `status` بدلًا من افتراض أن العلامة غير نشطة تعني وجود خطأ في الإعداد.

:::note ما الذي تثبته نبضة القلب فعليًا
نبضة القلب الناجحة تثبت أن مفتاح المشغِّل وقَّع في الوقت المحدد — ولا تثبت أن العقدة تشغِّل البرنامج الكامل باستمرار. تعامل معها كتوقيع استمرارية نشاط، لا كـ"تحقُّق من عقدة نشطة".
:::

:::note `last_heartbeat` هو ارتفاع كتلة، لا طابع زمني
إذا استعلمت عن سجل العقدة على السلسلة مباشرة، فإن `last_heartbeat` هو ارتفاع كتلة، والقيمة `0` تعني أن العقدة لم ترسل نبضة قلب بعد قط — تُبلِّغ السلسلة في هذه الحالة عن ارتفاع `registered_at` كبديل. قراءتها كحساب ساذج لزمن منقضٍ يجعل عقدة مسجَّلة حديثًا تبدو متأخرة بملايين الكتل.
:::

*أهلية المكافأة: احتفظ بترخيص نشط على السلسلة والحد الأدنى من الحصة المفوَّضة، سجِّل، ثم استمر في إثبات النشاط عبر نبضات القلب للبقاء فوق حدَّي نسبة التشغيل وفترة نبضات القلب اللازمين لاستمرار تدفق الحصة.*
...
يشغِّل توليد المفاتيح ← التوقيع ← التحقق ← كشف العبث (خمسة فحوص) ويخرج بقيمة غير صفرية عند أي فشل. هذه أسرع طريقة لاستبعاد وجود مشكلة في حزمة التوقيع المقاوم للحوسبة الكمية عند تشخيص مشكلات العقدة. راجع [إصدار SX](/light-node/sx-edition) لتفصيل الاختبار الذاتي الكامل.
