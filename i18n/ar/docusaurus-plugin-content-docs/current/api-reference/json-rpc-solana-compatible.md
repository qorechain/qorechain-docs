...
اعتبارًا من إصدار السلسلة v3.1.89 (22 أغسطس)، وعقب حادثة، أصبح مسار تنفيذ SVM **معطّلاً على مستوى الشبكة بالكامل بالنسبة إلى إرسال المعاملات** — أي معاملة تُعيد `code 11, "SVM module is disabled"`. ينطبق هذا على مستوى الشبكة بأكملها، وليس فقط عند نقاط النهاية العامة للقراءة فقط. قد تظل الطرق ذات طابع القراءة في الجدول أدناه (مثل `getBalance` و`getAccountInfo`) تستجيب، لكن تجنّب أي تكامل فعلي يُرسل معاملات SVM إلى أن يُعاد فتح هذا المسار — هذا التعطيل يتم وقت الترجمة (compile-time) وليس عبر معامل وقت التشغيل، لذا لا يمكن إعادة تفعيله بتصويت حوكمة؛ ومن المتوقع أن يظل معطّلاً إلى أن يُقرّه تدقيق خارجي.
:::
