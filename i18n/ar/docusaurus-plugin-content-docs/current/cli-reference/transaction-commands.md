...
:::note
اضبط `--chain-id qorechain-vladi` للبث على الشبكة الرئيسية الحية (إصدار السلسلة **v3.1.97**)، أو `--chain-id qorechain-diana` لشبكة الاختبار. إذا تم حذف هذه القيمة، يستخدم العميل `chain-id` من إعداداتك المحلية.
:::
