...
:::note
تُنفَّذ الاستعلامات مقابل العُقدة التي يشير إليها `--node`. استخدم نقطة نهاية RPC للشبكة الرئيسية **`qorechain-vladi`** (إصدار السلسلة **v3.1.80**) للبيانات الحيّة، أو نقطة نهاية لشبكة الاختبار **`qorechain-diana`** للاختبار. تستهدف القيمة الافتراضية `tcp://localhost:26657` عُقدة تشغّلها بنفسك.
:::
