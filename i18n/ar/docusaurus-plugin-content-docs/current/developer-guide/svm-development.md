...
:::note
تستخدم الأوامر أدناه شبكة **`qorechain-vladi`** الرئيسية، العاملة منذ 7 يونيو 2026 بإصدار السلسلة **v3.1.95**. استبدل `--chain-id qorechain-diana` للشبكة التجريبية.
:::

---

:::caution إرسال معاملات SVM معطّل حاليًا
اعتبارًا من إصدار السلسلة v3.1.89 (22 أغسطس)، وعقب حادثة وقعت، أصبح مسار تنفيذ SVM **معطّلًا على مستوى الشبكة بأكملها بالنسبة لإرسال المعاملات** — أي معاملة تُرسَل إلى `x/svm` (نشر برنامج، تنفيذ تعليمات، إنشاء حساب، تحويلات) تُعيد `code 11, "SVM module is disabled"`. ينطبق هذا على عقدتك الخاصة أيضًا وعلى النقاط العامة سواء بسواء. قد تستجيب طرق RPC ذات طابع القراءة، لكن لا تبنِ أو تختبر تكاملًا فعليًا مع SVM إلى أن يُعاد فتح المسار — هذا تعطيل وقت الترجمة، وليس معاملًا وقت التشغيل، لذا لا يمكن إعادة تفعيله عبر تصويت حوكمة؛ ومن المتوقع أن يبقى معطّلًا إلى أن يُخلي طرفه تدقيق خارجي.
:::
