...
:::caution حافظ على تحديث عقدتك — يلزم الإصدار v3.1.94 أو أحدث لإجراء مزامنة جديدة
يجب على العقد الكاملة مواكبة إصدار السلسلة المباشر للشبكة — ثبّت دائماً الملف التنفيذي الذي يشير إليه البيان (manifest)، ولا تُثبّت إصداراً قديماً بشكل ثابت. الحد الأدنى للعقدة التي تنضم من جديد (من genesis) أو التي تتعافى من توقف هو **v3.1.94 أو أحدث**، لسببين مستقلّين ومتراكمين: أصلح الإصدار v3.1.92 خللاً في قياس الغاز (gas-metering) كان يوقف إعادة التشغيل (replay) عند أول كتلة تحتوي على معاملة، كما تجاوزت الشبكة الرئيسية منذ ذلك الحين ترقية الحوكمة v3.1.94 (سقف أقصى صارم على الإصدار (emission)، طُبِّق عند الارتفاع 2,122,074) — والعقدة التي تفتقر إلى معالج (handler) هذه الترقية تتوقف مجدداً عند محاولة إعادة تشغيل تتجاوز هذا الارتفاع نفسه. الإصدار v3.1.97 هو الإصدار الموصى به حالياً (تحديث أمني متجدد؛ قيمة `minCompatible` هي `3.1.94`). تحقق دائماً من حقل `"version"` في البيان مقابل الحد الأدنى الحالي قبل الوثوق بـ `binary.url` الخاص به — إذ يُروَّج البيان بشكل مدروس (أولاً على شبكة الاختبار، ثم على الشبكة الرئيسية بعد فترة اختبار) وقد تأخر سابقاً عن الحد الأدنى للإصدار.
:::
