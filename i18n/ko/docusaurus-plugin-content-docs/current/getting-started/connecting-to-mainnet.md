...
**매니페스트가 실제로 무엇을 제공하고 있는지 신뢰하기 전에 반드시 확인하세요.** 매니페스트는 테스트넷에 먼저 반영된 뒤 안정화 기간을 거쳐 메인넷에 반영되는 방식으로 의도적으로 단계적으로 승격되므로, 위의 버전 하한선보다 뒤처질 수 있습니다. 이 글을 쓰는 시점에도 메인넷 매니페스트 자체가 아직 v3.1.92 이전 바이너리를 가리키고 있는데, 이는 바로 이 주의 문구가 새로 동기화할 때 사용하지 말라고 경고하는 바로 그 빌드입니다. `binary.url`을 신뢰하기 전에 매니페스트의 `"version"` 필드를 v3.1.92와 비교하세요. 아직 뒤처져 있다면, [qorechain-core GitHub 릴리스](https://github.com/qorechain/qorechain-core/releases)에서 v3.1.92 이상을 대신 받거나(태그의 체크섬도 동일한 방식으로 검증하세요), [소스에서 빌드](/developer-guide/building-from-source)하세요.
:::
