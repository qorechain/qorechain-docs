...
:::note
두 웹소켓 인터페이스 모두 **`qorechain-vladi`** 메인넷(체인 버전 **v3.1.95**에서 가동 중)과 **`qorechain-diana`** 테스트넷에서 사용할 수 있습니다. 아래의 로컬 엔드포인트는 직접 실행하는 노드를 기준으로 합니다. 원격으로 접속하려면 프로바이더의 메인넷 또는 테스트넷 호스트로 대체하십시오.
:::
