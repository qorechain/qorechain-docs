...
:::note
`qor_` 네임스페이스는 **`qorechain-vladi`** 메인넷(EVM 체인 ID **9801**, 체인 버전 **v3.1.97**에서 가동 중)과 **`qorechain-diana`** 테스트넷(EVM 체인 ID **9800**)에서 사용할 수 있습니다. 아래 예제는 로컬 노드를 가정합니다. 원격 접속의 경우 공급자의 메인넷 또는 테스트넷 엔드포인트로 대체하세요.
:::
