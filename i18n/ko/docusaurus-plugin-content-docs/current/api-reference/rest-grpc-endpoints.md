...
QoreChain은 프로그래밍 방식 접근을 위해 세 가지 기본 인터페이스를 제공합니다:
...
:::note
이 인터페이스들은 **`qorechain-vladi`** 메인넷(2026년 6월 7일부터 가동 중, 체인 버전 **v3.1.95**)과 **`qorechain-diana`** 테스트넷 모두에서 사용할 수 있습니다. 아래의 기본 URL은 로컬에서 실행 중인 노드를 기준으로 합니다. 공개 호스팅 엔드포인트(`rpc/api/evm/svm.qore.host` 및 해당 `-testnet` 변형)는 [네트워크](/appendix/networks#public-endpoints)에 정리되어 있습니다.
:::
...
| GET    | `/ai/v1/fraud/investigations`      | 진행 중인 모든 사기 조사 목록              |
| GET    | `/ai/v1/fraud/investigations/{id}` | 특정 사기 조사의 상세 정보를 반환 |
...
| GET    | `/qorechain/bridge/v1/validators`          | 등록된 브리지 검증인 목록       |
| GET    | `/qorechain/bridge/v1/validators/{address}`| 특정 브리지 검증인의 상세 정보  |
...
| GET    | `/bridge/v1/validators`             | 활성 브리지 검증인 목록                 |
...
| GET    | `/bridge/v1/estimate`               | 브리지 수수료 및 전송 시간 추정         |
...
| 메서드 | 엔드포인트                              | 설명                               |
| ------ | ------------------------------------- | ----------------------------------------- |
| GET    | `/reputation/v1/validators`           | 모든 검증인의 평판 점수      |
| GET    | `/reputation/v1/validators/{address}` | 특정 검증인의 평판 점수 |

## Cross-VM 모듈

| 메서드 | 엔드포인트                   | 설명                              |
| ------ | -------------------------- | ---------------------------------------- |
| GET    | `/crossvm/v1/message/{id}` | ID로 크로스 VM 메시지 조회       |
| GET    | `/crossvm/v1/pending`      | 대기열에 있는 보류 중인 크로스 VM 메시지 목록 |
| GET    | `/crossvm/v1/params`       | 현재 Cross-VM 모듈 파라미터       |

## 멀티레이어 모듈 {#multilayer-module}

체인 버전 **v3.1.80**부터 멀티레이어 모듈의 전체 쿼리 서비스가 grpc-gateway를 통해 `/qorechain/multilayer/v1/...` 접두사 아래에서 REST로 노출되며(이전에는 gRPC 전용), 두 개의 **상태 앵커 읽기 쿼리**가 포함됩니다: `anchor/{layer_id}`는 레이어의 최신 정산 앵커를 반환하고, `anchors/{layer_id}`는 해당 레이어의 앵커 이력을 반환합니다. 각 앵커는 정규(canonical) 필드에 대한 **ML-DSA-87 (Dilithium-5)** 서명을 포함하므로, 클라이언트가 앵커를 가져와 독립적으로 검증할 수 있습니다 — 이는 Rollup Development Kit의 [정산 영수증](/rollups/settlement-receipts)의 온체인 기반입니다.

| 메서드 | 엔드포인트                                        | 설명                                       |
| ------ | ----------------------------------------------- | ------------------------------------------------- |
| GET    | `/qorechain/multilayer/v1/params`               | 현재 Multilayer 모듈 파라미터              |
...
`StateAnchorView`는 `layer_id`, `layer_height`, `state_root`, `validator_set_hash`, `main_chain_height`, `anchored_at`, `pqc_aggregate_signature`, `transaction_count`, `compressed_state_proof`를 포함합니다. 서명되는 정규 메시지는 `layer_id || layer_height || state_root || validator_set_hash`이며, 레이어 생성자가 등록한 PQC 키에 대해 검증됩니다.
...
| GET    | `/multilayer/v1/params`        | 현재 Multilayer 모듈 파라미터         |
...
| GET    | `/svm/v1/program/{address}` | 지정한 프로그램 주소에 배포된 프로그램 정보 |

## RL 합의 모듈

PRISM 튜닝 파라미터와 강화 학습 에이전트 상태가 이 모듈을 통해 노출됩니다.
...
| 메서드 | 엔드포인트                       | 설명                          |
| ------ | ------------------------------ | ------------------------------------ |
| GET    | `/qorechain/burn/v1/params`    | 현재 Burn 모듈 파라미터       |
| GET    | `/qorechain/burn/v1/stats`     | 모든 채널의 소각 통계  |
| GET    | `/qorechain/burn/v1/records`   | 소각 기록 목록                   |
...
| GET    | `/burn/v1/params` | 현재 Burn 모듈 파라미터      |
...
| GET    | `/inflation/v1/params` | 현재 Inflation 모듈 파라미터 |
...
| GET    | `/rdk/v1/batch/{id}/{index}` | 특정 정산 배치 조회 |
| GET    | `/rdk/v1/batches/{id}`       | 특정 롤업의 배치 목록   |
| GET    | `/rdk/v1/blob/{id}/{index}`  | 특정 DA 블롭 조회          |
...
체인 버전 **v3.1.85**부터 추상 계정 쿼리 서비스가 grpc-gateway를 통해 `/qorechain/abstractaccount/v1/...` 접두사 아래에서 REST로 노출되며, 하드코딩 없이 [인증자(authenticator)](/developer-guide/account-abstraction#authenticators) 범위를 검증하는 데 사용되는 **권한 스키마**도 포함됩니다.

| 메서드 | 엔드포인트                                              | 설명                                            |
| ------ | ----------------------------------------------------- | ------------------------------------------------------ |
| GET    | `/qorechain/abstractaccount/v1/config`                | 모듈 구성(기능 프로브: `enabled`)        |
| GET    | `/qorechain/abstractaccount/v1/accounts`              | 추상 계정 목록                                |
| GET    | `/qorechain/abstractaccount/v1/accounts/{address}`    | 주소의 추상 계정 상태                  |
| GET    | `/qorechain/abstractaccount/v1/permission_schema`     | 권한 분류 체계, msg→permission 맵, 위임 불가 msg 목록 |
...
| GET    | `/abstractaccount/v1/params`            | 현재 Abstract Account 모듈 파라미터   |
...
| 메서드 | 엔드포인트                             | 설명                               |
| ------ | ------------------------------------ | ----------------------------------------- |
| GET    | `/gasabstraction/v1/accepted-tokens` | 가스 지불에 허용되는 토큰 목록     |
| GET    | `/gasabstraction/v1/params`          | 현재 Gas Abstraction 모듈 파라미터 |
...
모든 REST 및 gRPC 엔드포인트는 기본적으로 인증되지 않습니다. 프로덕션 배포에서는 노드 앞에 리버스 프록시(예: Nginx 또는 Caddy)를 두어 TLS 종료와 접근 제어를 처리하십시오.
