...
:::note
라이브 메인넷(체인 버전 **v3.1.80**)에 브로드캐스트하려면 `--chain-id qorechain-vladi`를 설정하고, 테스트넷에 브로드캐스트하려면 `--chain-id qorechain-diana`를 설정하세요. 생략하면 클라이언트는 로컬 설정의 `chain-id`를 사용합니다.
:::
...
| `--commission-max-change-rate` | string | 최대 일일 수수료 변경율                    |
| `--min-self-delegation`        | string | 필요한 최소 자기 위임                      |

### edit-validator

기존 검증인의 설명 또는 수수료를 편집합니다.
...
cosmos 트랜잭션 경로는 기본적으로 하이브리드 서명을 요구합니다(`hybrid_signature_mode = required`). `gen-key` 및 `cosign` 명령어는 고전적인 secp256k1 서명과 함께 cosmos 경로에서 트랜잭션을 처리하는 데 필요한 Dilithium-5 (ML-DSA-87) 키와 `PQCHybridSignature` 확장을 생성합니다.
...
트랜잭션에 Dilithium-5 공동 서명을 `PQCHybridSignature` 확장으로 첨부하여 하이브리드(secp256k1 + ML-DSA-87) 트랜잭션을 생성합니다. 기본 `required` 강제 모드에서 cosmos 경로 트랜잭션에 필요합니다. 표준 CosmJS / 릴레이어 도구는 트랜잭션을 처리하기 위해 이 확장을 생성해야 합니다. QoreChain SDK의 `buildHybridTx`(`includePqcPublicKey` 포함)가 동등한 작업을 수행합니다.
...
확장된 메타데이터 및 증명과 함께 PQC 키를 등록합니다.
...
xQORE를 다시 QOR로 잠금 해제합니다. 조기 잠금 해제는 패널티 등급에 따라 패널티가 발생할 수 있습니다.
...
단일 서명 트랜잭션으로 체인의 브릿지를 활성화하거나 재구성합니다(체인 버전 **v3.1.80**부터 사용 가능). `bridge_admin` 키 또는 `qcb_bridge` 라이선스가 필요하며, 거버넌스 제안이나 체인 업그레이드는 필요하지 않습니다. 컨트랙트 주소, 확인 수, 아키텍처 및 상태를 설정합니다.
...
체인의 활성 검증자를 선택하고 신뢰 루트를 설치합니다(역시 `bridge_admin`으로 게이트됨).
...
실행 환경 간(EVM, CosmWasm, SVM) 크로스 VM 메시지를 전송합니다.
...
| 플래그       | 타입   | 설명                                              |
| ------------ | ------ | ------------------------------------------------- |
| `--accounts` | string | 명령에 대한 쉼표로 구분된 계정 공개 키            |
...
Rollup Development Kit으로 새 롤업을 등록합니다.
...
정산 배치에 대한 사기 챌린지를 제출합니다(옵티미스틱 롤업).
...
:::note
롤업 출금 및 크로스 레이어 정산도 `rdk` 트랜잭션 그룹 아래에 노출됩니다(예: 최종화된 배치에 대해 증명된 출금을 정산하는 `execute-withdrawal` 명령어). 정확한 인수와 플래그는 롤업의 정산 유형 및 DA 구성에 따라 달라집니다. 이러한 트랜잭션을 구성하기 전에 권위 있는 명령어 표면에 대해서는 **Rollup Development Kit** 문서를 참조하세요.
:::
...
PRISM은 합의 매개변수를 조정하는 강화 학습 레이어입니다. 이 명령어들은 PRISM 에이전트를 제어합니다. CLI 모듈 이름 `rlconsensus`와 그 하위 명령어는 그대로 유지됩니다.
...
PRISM 에이전트에 대한 보상 가중치 구성을 업데이트합니다.
