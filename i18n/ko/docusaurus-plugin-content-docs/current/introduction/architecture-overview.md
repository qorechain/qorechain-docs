...
QoreChain은 세 가지 주요 프로세스 — 체인 노드, AI 사이드카, 블록 인덱서 — 로 구성된 모듈식 블록체인 노드이며, Postgres 데이터베이스로 뒷받침되고 Prometheus 및 Grafana를 통해 모니터링됩니다. 메인넷(`qorechain-vladi`, EVM 체인 ID **9801**)은 2026년 6월 7일부터 체인 버전 **v3.1.80**로 가동 중이며, 병렬 테스트넷(`qorechain-diana`, EVM 체인 ID **9800**)이 함께 운영됩니다. 체인은 Cosmos SDK v0.53 위에 구축되었습니다. 다음 다이어그램은 고수준 컴포넌트 레이아웃을 보여줍니다.
