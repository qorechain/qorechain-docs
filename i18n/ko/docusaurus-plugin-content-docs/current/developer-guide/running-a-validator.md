...
:::note
이 가이드는 **`qorechain-vladi`** 메인넷(EVM 체인 ID **9801**)을 대상으로 하며, 2026년 6월 7일부터 체인 버전 **v3.1.97**로 운영되고 있습니다. **`qorechain-diana`** 테스트넷(EVM 체인 ID **9800**)은 실제 배포 전에 설정을 미리 연습해보는 용도로 권장됩니다. 대상 네트워크에 맞는 `--chain-id`로 대체하세요.
:::
