...
체인 버전 v3.1.89(8월 22일) 기준으로, 한 건의 사고 이후 SVM 실행 레인은 **트랜잭션 제출에 대해 네트워크 전역에서 비활성화**되었습니다 — `x/svm`으로 전송되는 모든 트랜잭션(프로그램 배포, 명령어 실행, 계정 생성, 전송)은 `code 11, "SVM module is disabled"`를 반환합니다. 이는 여러분 자신의 노드는 물론 공개 엔드포인트에도 동일하게 적용됩니다. 읽기 성격의 RPC 메서드는 계속 응답할 수 있지만, 이 레인이 다시 열리기 전까지는 실제 SVM 연동을 구축하거나 리허설하지 마세요.
:::
