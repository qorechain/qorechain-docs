...
|---|---|
| **Firefox** | **0.2.2** |
| **Chrome / Chromium** | **0.1.5** (0.1.9 제출 완료, 아직 심사 중 — 해당 심사가 끝날 때까지 등록 정보는 신규 제출이 잠겨 있어, 0.2.2는 아직 이곳에 제출되지 않았습니다) |
| **Safari (macOS)** | **QoreX Wallet** macOS 앱에 포함되어 배포되며, 이 앱은 자체 `1.x` 번호 체계를 사용합니다 — Mac App Store에서는 현재 **1.3**이 제공됩니다(확장 프로그램 **0.2.2** 포함) |

최신 기능이 아직 사용 중인 브라우저에는 배포되지 않았을 수 있습니다 — 이 문서에서 설명하는 내용이 실제로 사용 가능하다고 가정하기 전에 위 표를 먼저 확인하세요. Dashboard가 확장 프로그램을 업데이트해야 한다고 안내한다면, 이는 해당 작업(대개 스테이킹의 경우 0.2.2)에 필요한 특정 최소 버전을 의미하는 것이지, 여러분의 빌드가 전반적으로 오래되었다는 뜻이 아닙니다.
