...
|---|---|
| **Firefox** | **0.2.6** |
| **Safari (macOS)** | **QoreX Wallet** macOS 앱에 포함되어 배포되며, 이 앱은 자체 `1.x` 번호 체계를 사용합니다 — Mac App Store에서는 현재 **1.6**이 제공됩니다(확장 프로그램 **0.2.6** 포함) |
| **Chrome / Chromium** | 8월 하순 기준 오랜 스토어 심사에 걸려 있었습니다 — 여기 적힌 숫자를 믿기보다는 [Chrome 웹 스토어 등록 정보](https://chromewebstore.google.com/detail/qorex/cflpnggbnnifibblifgbeobogdpfjpcg)에서 현재 버전을 직접 확인하세요 |

최신 기능이 아직 사용 중인 브라우저에는 배포되지 않았을 수 있습니다 — 이 문서에서 설명하는 내용이 실제로 사용 가능하다고 가정하기 전에 위 표를 먼저 확인하세요. Dashboard가 확장 프로그램을 업데이트해야 한다고 안내한다면, 이는 해당 작업(예: 스테이킹의 경우 0.2.2)에 필요한 특정 최소 버전을 의미하는 것이지, 여러분의 빌드가 전반적으로 오래되었다는 뜻이 아닙니다.
...
**0.2.2**에서는 [확장 프로그램 자체에서의 스테이킹](#stake)이 추가되었습니다 — 커미션이 표시된 검증인 목록, 여러분의 스테이킹 총액, 대기 중인 보상, 위임/언스테이킹/청구를 다루는 자체 Stake 화면; 모바일 앱과 동일하게 [하나의 복구 문구로 여러 계정](#wallet)을 만드는 기능; **Dashboard**의 스테이킹 버튼이 실제로 확장 프로그램에 도달하도록 하는 수정(이전에는 확장 프로그램에서만 생성된 지갑은 Dashboard를 통한 스테이킹을 전혀 할 수 없었습니다 — [Dashboard 브리지](#dashboard-bridge) 참고); 브라우저에서 작동하는 @handle 청구 기능; 그리고 팝업 하단에 표시되는 빌드 번호.
...
:::note 하나의 복구 문구로 여러 계정 (0.2.2부터)
확장 프로그램은 이제 모바일 앱과 동일하게, 하나의 복구 문구로 여러 계정을 만들고 그 사이를 전환할 수 있습니다 — 이미 적어 둔 문구로 모든 계정을 복구할 수 있습니다. 전환하면 전송, 스테이킹, 받기, @handle 등 모든 것이 활성화된 계정을 따라 함께 이동합니다. 포트폴리오, Q-Day 스캐너, 소셜 복구, Legacy Protocol, 결제 요청, 기기 연결은 여전히 모바일 전용 기능입니다 — 전체 비교는 [QoreX Wallet](/qorex/overview#platform-availability)을 참고하세요.
:::
...
QoreX가 확인 결과를 사용하기 전에 두 가지 방식으로 검증됩니다. 확장 프로그램에 내장된 신뢰 키로 검증하는 레지스트리 증명, 그리고 청구 내용에 대한 핸들 소유자 본인의 서명입니다. 둘 중 하나라도 실패하면 응답은 즉시 거부됩니다 — QoreX는 검증되지 않은 주소를 대신 보여주지 않습니다. 특정 핸들로 처음 전송할 때 QoreX는 확인된 주소를 기억(고정)해 둡니다. 이후 그 핸들이 **다른** 주소로 확인되면 QoreX는 진행을 멈추고 이전 주소와 새 주소를 모두 온전히 보여주어 계속할지 여부를 직접 판단하게 합니다. 이 기억은 **브라우저별로** 유지됩니다 — 같은 핸들이라도 다른 브라우저나 컴퓨터에서 처음 결제하면 그곳에서도 새로운 것으로 표시됩니다. 이는 오류가 아니라 예상된 동작입니다.

## 받기 {#receive}

팝업에서 **Receive**를 탭하면 `qor1…` 주소가 QR 코드(QoreChain 아이콘이 삽입됨)와 복사 버튼으로 표시됩니다 — 휴대폰으로 스캔하거나 주소를 직접 붙여넣으세요.

## 확장 프로그램에서 스테이킹하기 {#stake}

**0.2.2**부터 팝업에 자체 **Stake** 화면이 생겨, 확장 프로그램에서만 생성된 지갑도 모바일 앱 없이 스테이킹 보상을 받을 수 있습니다.

1. 팝업을 열고 **Stake**로 이동합니다.
2. 이 화면은 커미션이 표시된 활성 검증인 목록, 현재 스테이킹 총액, 청구 대기 중인 보상을 보여줍니다. 네트워크가 **jail(수감)** 처리한 검증인은 목록에서 제외됩니다 — 그런 검증인에게 위임하는 것은 결코 바람직하지 않기 때문입니다.
3. 위임하려면 검증인과 금액을 선택한 뒤 확인합니다. QoreX는 Send와 마찬가지로 필수 하이브리드 포스트 양자 서명으로 서명합니다.
4. **Unstake(언스테이킹)**와 **claim(청구)**도 같은 화면에서 처리됩니다. 언스테이킹을 시작하면 21일의 언본딩 기간이 시작됩니다 — 이것이 의미하는 바는 [스테이킹 및 위임](/user-guide/staking-and-delegation)을 참고하세요.
5. **0.2.6**부터는 **스테이킹을 다른 검증인으로 옮기는 것**(재위임)도 가능합니다 — 언본딩 대기가 없고, 페널티도 없으며, 보상은 그 과정 내내 계속 쌓입니다. 작동 방식은 [검증인 간 스테이킹 이동](/qorex/portfolio-and-staking#move-stake)을 참고하세요(앱과 확장 프로그램의 동작 방식은 동일합니다).

스테이킹, 위임, 보상은 전적으로 **Native** 레인에서만 이루어지며, EVM 프리컴파일을 통해서는 절대 이루어지지 않습니다.

### Dashboard 스테이킹 요청 승인하기 {#stake-dashboard}

QoreChain [Dashboard](/dashboard/staking-and-validators)는 스테이킹 요청을 구성하지만 직접 서명하지는 못합니다 — 여러분의 키는 절대 확장 프로그램의 볼트를 벗어나지 않습니다. Dashboard에서 **Continue in QoreX**를 클릭하면, 요청이 확장 프로그램에서 열려 검토(검증인과 금액)하고 Send와 똑같이 승인할 수 있습니다. 이 연결은 0.2.1에서 한 차례 끊어졌었습니다(확장 프로그램이 최신 게시 빌드였음에도 스스로를 "너무 오래됨"으로 보고했는데, 실제 원인은 버전이 오래된 것이 아니라 내부 중계 단계 누락이었습니다). **0.2.2**부터 수정되었습니다. 더 오래된 빌드를 사용 중이라면 [브라우저별로 어떤 버전이 서비스 중인가](#versions)를 참고하세요.

:::note 트랜잭션이 성공 대신 "downgraded(강등됨)"로 표시되는 경우
Dashboard는 트랜잭션을 깔끔한 성공 대신 **downgraded(강등됨)**로 표시할 때가 있습니다. 이는 자금은 실제로 이동했지만 해당 트랜잭션에 대한 포스트 양자 서명 계층이 체인에서 확인되지 않았다는 의미입니다 — 여러분이 잘못한 것도 아니고, 여러분 쪽에서 고칠 수 있는 것도 아닙니다. 이것은 저희 쪽의 결함이니, 저희가 조사할 수 있도록 지원팀에 알려 주세요. 이 메시지는 읽고 신고할 시간을 드리기 위해 의도적으로 사라지지 않고 화면에 계속 표시됩니다.
:::
...
### Dashboard 브리지 (v0.1.5, v0.2.2에서 확장) {#dashboard-bridge}
...
**0.2.2**는 `native:executeRequest`를 추가합니다. 이 메서드는 [스테이킹](#stake-dashboard)을 포함해 Dashboard가 제안하는 요청 전체를 받아들이며, QoreX가 다른 모든 곳에서 사용하는 것과 동일한 공유 파서로 검증합니다. 네트워크 불일치, 낯선 출처, 여러분의 것이 아닌 주소, 알 수 없는 요청 종류, 또는(스테이킹 요청에는 있을 수 없는) `toAddress`를 담은 스테이킹 요청의 경우 거부됩니다.
