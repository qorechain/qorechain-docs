...
:::note
아래 명령어는 **`qorechain-diana`** 테스트넷(EVM 체인 ID **9800**)을 기준으로 합니다. 메인넷(**`qorechain-vladi`**, EVM 체인 ID **9801**)은 2026년 6월 7일부터 체인 버전 **v3.1.95**로 운영 중이며, 메인넷에서 스테이킹할 때는 **메인넷 연결** 페이지에 있는 메인넷 체인 ID와 엔드포인트로 대체해서 사용하세요.
:::
...
재위임에는 **패널티도, 그 자체의 락도 없습니다** — 스테이크가 본딩 풀을 벗어나지 않으므로 보상이 끊기지 않으며, 언제든 다시 옮길 수 있습니다. 21일 언본딩 기간의 적용 대상이 전혀 아니며, 그 기간은 `unbond`에만 적용됩니다.

:::caution 진짜 제한은 쿨다운이 아니라 건수입니다
위임자는 정확히 동일한 (위임자, 원본 검증인, 대상 검증인) 경로에 대해 최대 **7건의 재위임 진행 항목**을 동시에 가질 수 있습니다 — 각 항목은 만료되면 개별적으로 정리되어 슬롯이 다시 확보됩니다. 이는 일반적인 사용에서는 사실상 도달하지 않는 상한선일 뿐, "다시 재위임하려면 대기해야 한다"는 규칙이 아닙니다. 다른 검증인으로/으로부터의 재위임은 자유롭게 할 수 있고, 슬롯이 하나 비면 동일한 경로로도 다시 재위임할 수 있습니다.
:::
...
스테이킹 보상은 두 가지 원천에서 지급됩니다. 프로토콜의 상한이 설정된 발행(emission) 예산(현재 상한은 2026년 8월 26일 거버넌스 변경 이후 적용 중이며, [토크노믹스](/architecture/tokenomics#staking-reward-schedule) 참고)과, 모든 거래 수수료 중 스테이커 몫입니다.
...
* 재위임은 즉시 처리되며 패널티도 락도 없습니다 — 유일한 제한은 정확히 동일한 경로에서 동시에 진행 중인 재위임 건수가 7건으로 제한된다는 것뿐이며, 일반적인 사용에서는 이 상한에 도달하지 않습니다.
