...
:::caution
RDK와 롤업 정산 레이어는 활발히 발전 중인 기능입니다. 이 섹션 전반에 설명된 정산 모드, 증명 시스템, 프리셋, 기능별 성숙도는 변경될 수 있는 설계 의도로 간주하고, 메인넷(**`qorechain-vladi`**, EVM 체인 ID **9801**, 체인 버전 **v3.1.80**)을 대상으로 하기 전에 **`qorechain-diana`** 테스트넷에서 모든 배포를 검증하세요.
:::
