...
등록과 위임을 마친 뒤에는, 자격을 유지하는 것은 곧 라이브 상태를 유지하는 문제가 됩니다. 노드는 최소 **80% 가동률**을 유지해야 하며, 대략 **1,000블록(~39분) 간격**으로 하트비트 라이브니스 증명을 계속 제출해야 합니다. 하트비트를 놓친 뒤에는 비활성으로 표시되기까지 **약 100블록(~4분)의 유예 기간**이 주어집니다. 비활성으로 표시된 노드는 다시 라이브니스를 증명할 때까지 분배분을 받지 못합니다.
