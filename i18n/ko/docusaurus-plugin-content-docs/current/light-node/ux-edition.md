...
:::danger 대시보드에는 인증이 없습니다
포트 8420은 로컬호스트뿐 아니라 **모든 인터페이스**에서 수신 대기하며, 대시보드에는 **로그인도 접근 제어도 없습니다**. 네트워크에서 이 포트에 접근할 수 있는 사람은 누구나 회원님의 설정, 위임, 보상을 읽을 수 있습니다. **공개적으로 노출하지 마세요.** 완전히 신뢰할 수 있는 사설 네트워크가 아닌 곳에서 실행하기 전에는 루프백에만 바인딩하거나, 인증을 요구하는 리버스 프록시 뒤에 두세요.
:::
