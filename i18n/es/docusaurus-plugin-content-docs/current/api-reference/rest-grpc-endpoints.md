...
:::note
Estas interfaces están disponibles tanto en la mainnet **`qorechain-vladi`** (activa desde el 7 de junio de 2026 en la versión de cadena **v3.1.85**) como en la testnet **`qorechain-diana`**. Las URL base que aparecen a continuación asumen un nodo en ejecución local; los endpoints públicos alojados (`rpc/api/evm/svm.qore.host` y sus variantes `-testnet`) se enumeran en [Redes](/appendix/networks#public-endpoints).
:::
...
| GET    | `/ai/v1/stats`                     | Estadísticas agregadas de procesamiento de AI                      |
| GET    | `/ai/v1/fee-estimate`              | Estimación de comisión de gas asistida por AI para una transacción |
...
A partir de la versión de cadena **v3.1.77**, el estado de solo lectura del módulo bridge se expone por REST mediante grpc-gateway bajo el prefijo `/qorechain/bridge/v1/...` (antes solo estaba disponible por gRPC). Estos endpoints sirven JSON real de la cadena a través de HTTP para exploradores y telemetría de nodos ligeros. La `config` del bridge reporta, por ejemplo, `min_validators=10` y `threshold=7`.
...
| GET    | `/qorechain/bridge/v1/operations`          | Enumera las operaciones de bridge               |
...
| GET    | `/bridge/v1/operations`             | Enumera las operaciones de bridge recientes                 |
...
| GET    | `/svm/v1/account/{address}` | Información de la cuenta SVM para una dirección dada                     |
...
A partir de la versión de cadena **v3.1.77**, el estado de solo lectura del módulo burn se expone por REST mediante grpc-gateway bajo el prefijo `/qorechain/burn/v1/...` (antes solo estaba disponible por gRPC). Estos endpoints sirven JSON real de la cadena a través de HTTP para exploradores y telemetría de nodos ligeros. Las `stats` de quema incluyen, por ejemplo, `gas_burn_rate=0.30`.
...
| GET    | `/qorechain/burn/v1/milestone` | Progreso del hito de quema                    |
...
A partir de la versión de cadena **v3.1.85**, el servicio de consulta de abstract-account se expone por REST mediante grpc-gateway bajo el prefijo `/qorechain/abstractaccount/v1/...`, incluido el **esquema de permisos** utilizado para validar los alcances de los [autenticadores](/developer-guide/account-abstraction#authenticators) sin valores codificados de forma fija.

| Método | Endpoint                                              | Descripción                                                   |
| ------ | ----------------------------------------------------- | -------------------------------------------------------------- |
| GET    | `/qorechain/abstractaccount/v1/config`                | Configuración del módulo (sonda de funcionalidad: `enabled`)    |
| GET    | `/qorechain/abstractaccount/v1/accounts`              | Enumera las cuentas abstractas                                 |
| GET    | `/qorechain/abstractaccount/v1/accounts/{address}`    | Estado de la cuenta abstracta para una dirección               |
| GET    | `/qorechain/abstractaccount/v1/permission_schema`     | Taxonomía de permisos, mapa msg→permiso, msgs no delegables    |
...
Todos los endpoints REST y gRPC no requieren autenticación de forma predeterminada. Para despliegues en producción, coloque un proxy inverso (por ejemplo, Nginx o Caddy) delante del nodo para gestionar la terminación TLS y el control de acceso.
