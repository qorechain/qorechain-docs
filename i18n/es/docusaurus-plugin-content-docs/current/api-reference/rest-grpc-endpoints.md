...
:::note
Estas interfaces están disponibles tanto en la mainnet **`qorechain-vladi`** (en producción desde el 7 de junio de 2026 en la versión de cadena **v3.1.97**) como en la testnet **`qorechain-diana`**. Las URLs base a continuación asumen un nodo ejecutándose localmente; los endpoints públicos alojados (`rpc/api/evm/svm.qore.host` y sus variantes `-testnet`) se listan en [Redes](/appendix/networks#public-endpoints).
:::
