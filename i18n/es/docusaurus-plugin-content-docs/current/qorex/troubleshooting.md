...
| **Una cuenta con fondos desapareció de "Tus cuentas"** | No se ha perdido nada: cada cuenta se vuelve a derivar a partir de tu frase de recuperación, así que se trata de un problema de visualización, no de pérdida de datos. Este fue un error real, ya corregido, en las compilaciones móviles anteriores a la **1.0.9**: una lectura fallida momentánea del almacenamiento seguro del dispositivo (probablemente justo al desbloquear) podía mostrar una lista de cuentas incompleta. Actualiza a la versión 1.0.9 o posterior; si ya tienes esa versión y esto ocurre, cerrar y volver a abrir la app debería hacer que reaparezca — tocar "añadir una cuenta" crearía una cuenta **nueva** en lugar de restaurar la que falta, así que evita hacerlo hasta que la cuenta existente reaparezca. |
