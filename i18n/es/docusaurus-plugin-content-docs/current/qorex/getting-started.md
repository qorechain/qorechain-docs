...
- **iOS** — disponible para pruebas a través de **TestFlight** si quieres probarlo; la publicación en la App Store todavía está en revisión. Encuentra el enlace de invitación vigente en [qorechain.io](https://qorechain.io).
:::

## Antes de empezar: protege tu dispositivo

Un monedero QoreX solo se puede crear o importar cuando tu dispositivo tiene configurado un **factor de desbloqueo fuerte**. Eso es lo que protege tus claves en la bóveda de hardware. Cualquiera de las siguientes opciones sirve:
...
El desbloqueo facial 2D basado en cámara (presente en algunos dispositivos, por ejemplo ciertos modelos de Samsung) cuenta como un biométrico *débil*. Si es lo único que tienes, QoreX se apoya en el **PIN / patrón** que hay detrás — y el panel del sistema lo ofrece automáticamente, así que sigues estando cubierto.
:::

Si no hay ningún factor fuerte registrado, los botones de crear/importar permanecen desactivados y la pantalla explica qué debes activar. Configura Face ID, una huella dactilar o un bloqueo de pantalla en los ajustes de tu sistema y vuelve después a QoreX.

## Primer inicio

La aplicación se abre en la pantalla de incorporación **solo cuando no existe ningún monedero en el dispositivo**. Una vez que tienes un monedero, todos los inicios posteriores van directamente a la pestaña Inicio (Monedero). Ver los saldos no requiere biometría; **firmar una transacción siempre la requiere**.
...
Cualquiera que tenga tus 24 palabras controla tus fondos, y nadie — ni siquiera QoreChain Association — puede recuperarlos por ti. Nunca escribas tu frase en un sitio web, no la compartas y no la guardes en una captura de pantalla ni en una nota en la nube.
:::
...
- [Portafolio y staking](/qorex/portfolio-and-staking) — sigue tus activos y obtén recompensas por staking.
