...
:::note
Establece `--chain-id qorechain-vladi` para transmitir contra la mainnet en vivo (versión de la cadena **v3.1.95**), o `--chain-id qorechain-diana` para la testnet. Si se omite, el cliente usa el `chain-id` de tu configuración local.
:::
...
Edita la descripción o la comisión de un validador existente.
...
La ruta de transacción cosmos requiere una firma híbrida por defecto (`hybrid_signature_mode = required`). Los comandos `gen-key` y `cosign` producen la clave Dilithium-5 (ML-DSA-87) y la extensión `PQCHybridSignature` necesarias para transaccionar en la ruta cosmos junto con la firma clásica secp256k1.
...
Desbloquea xQORE de vuelta a QOR. Un desbloqueo anticipado puede incurrir en penalizaciones según el nivel de penalización.
...
Inicia un depósito de puente desde una cadena externa.
...
Inicia un retiro de puente hacia una cadena externa.
...
Selecciona el verificador activo de una cadena e instala su raíz de confianza (también restringido a `bridge_admin`).
...
Envía una impugnación por fraude contra un lote de liquidación (rollups optimistas).
...
Finaliza manualmente un lote que ha superado la ventana de impugnación.
...
PRISM es la capa de aprendizaje por refuerzo que ajusta los parámetros de consenso. Estos comandos controlan el agente PRISM; el nombre del módulo CLI `rlconsensus` y sus subcomandos se conservan textualmente.
...
Valores de modo: `0` (apagado), `1` (observar), `2` (sugerir), `3` (automático).
...
Actualiza la configuración de la política del agente PRISM (solo gobernanza).
...
Actualiza la configuración de los pesos de recompensa para el agente PRISM.
