...
QoreChain esegue un ambiente di esecuzione completamente compatibile con EVM sul QoreChain EVM Engine, che consente di distribuire e interagire con smart contract Solidity utilizzando strumenti familiari. Il modulo EVM espone un'interfaccia JSON-RPC sulla **porta 8545** (WebSocket sulla **8546**) che supporta i flussi di lavoro di sviluppo Ethereum standard.

:::note
Gli esempi seguenti fanno riferimento alla mainnet **`qorechain-vladi`** (Chain ID EVM **9801**), attiva dal 7 giugno 2026 con versione della chain **v3.1.92**. Per la testnet **`qorechain-diana`**, usa il Chain ID EVM **9800**.
:::
...
| ------------------------ | ----------------------------- | ------------------------- |
| Nome rete                | QoreChain (qorechain-vladi)  | QoreChain Diana           |
| URL RPC                  | `http://localhost:8545`       | `http://localhost:8545`   |
| Chain ID                 | `9801`                        | `9800`                    |
| Simbolo valuta           | `QOR`                         | `QOR`                     |
| URL Block Explorer       | *(usa l'explorer ufficiale della mainnet)* | *(lascia vuoto per la testnet locale)* |
...
QoreChain utilizza un modello a **base fee dinamica EIP-1559** per le transazioni EVM:

* La base fee si adegua per ogni blocco in base all'utilizzo
...
Il token nativo QOR ha **6 cifre decimali** (`uqor`), mentre l'EVM prevede **18 cifre decimali**. Il modulo `x/precisebank` gestisce la conversione in modo trasparente:
...
Il modulo `x/erc20` fornisce la registrazione automatica di **coppie di token** tra le denominazioni native Cosmos SDK e i contratti ERC-20:
...
* La conversione è bidirezionale e viene gestita a livello di protocollo
...
## Compatibilità PQC ed EVM

Le transazioni EVM utilizzano firme **ECDSA classiche (secp256k1)** per garantire la piena compatibilità con gli strumenti, i wallet e le librerie Ethereum esistenti. Questo assicura che MetaMask, Hardhat, Foundry, ethers.js e tutti gli strumenti EVM standard funzionino senza modifiche.

Per la sicurezza post-quantistica all'interno dell'EVM:

* Usa il **precompile PQC Verify** (`0x0000...0A01`) per verificare on-chain le firme ML-DSA-87 da Solidity. Vedi [Precompile EVM](/developer-guide/evm-precompiles).
* I **messaggi cross-VM** dall'EVM verso CosmWasm o SVM possono essere firmati con PQC a livello di transazione Cosmos SDK.
* Gli account possono opzionalmente registrare chiavi pubbliche PQC tramite `x/pqc` per la sicurezza ibrida.

---

## Namespace personalizzato `qor_` {#custom-qor_-namespace}

QoreChain estende il JSON-RPC con un namespace `qor_` per le query specifiche della chain:

| Metodo                       | Descrizione                                                        |
| ----------------------------- | ------------------------------------------------------------------- |
| `qor_getPQCKeyStatus`         | Verifica se un account ha una chiave pubblica PQC registrata        |
| `qor_getAIStats`              | Recupera le statistiche del motore AI (conteggio anomalie, distribuzione del rischio) |
...
| `qor_getAbstractAccount`      | Recupera la configurazione dell'account astratto                    |
...
* [Precompile EVM](/developer-guide/evm-precompiles) — Accedi alle funzionalità PQC, AI e cross-VM da Solidity
* [Interoperabilità cross-VM](/developer-guide/cross-vm-interoperability) — Chiama contratti CosmWasm e SVM dall'EVM
