...
:::note
Le query vengono eseguite sul nodo indicato da `--node`. Usa un endpoint RPC mainnet **`qorechain-vladi`** (versione della chain **v3.1.97**) per i dati live, oppure un endpoint testnet **`qorechain-diana`** per i test. Il valore predefinito `tcp://localhost:26657` punta a un nodo eseguito da te.
:::
