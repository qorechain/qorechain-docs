...
:::note
Queste interfacce sono disponibili sia sulla mainnet **`qorechain-vladi`** (attiva dal 7 giugno 2026 sulla versione di chain **v3.1.95**) sia sulla testnet **`qorechain-diana`**. Gli URL di base riportati di seguito presuppongono un nodo in esecuzione locale; gli endpoint pubblici ospitati (`rpc/api/evm/svm.qore.host` e le loro varianti `-testnet`) sono elencati in [Reti](/appendix/networks#public-endpoints).
:::
...
| GET    | `/ai/v1/fee-estimate`              | Stima delle commissioni gas assistita dall'AI per una transazione |
...
Restano disponibili i seguenti endpoint con percorso abbreviato:
...
| GET    | `/bridge/v1/limits/{chain}`         | Limiti di frequenza e soglie per una chain collegata    |
...
| Metodo | Endpoint                   | Descrizione                                     |
| ------ | -------------------------- | ----------------------------------------------- |
| GET    | `/crossvm/v1/message/{id}` | Recupera un messaggio cross-VM per ID           |
...
A partire dalla versione di chain **v3.1.80**, il servizio di query completo del modulo multilayer è esposto via REST tramite grpc-gateway sotto il prefisso `/qorechain/multilayer/v1/...` (in precedenza solo gRPC), incluse due **query di lettura degli state anchor**: `anchor/{layer_id}` restituisce l'anchor di settlement più recente per un layer, e `anchors/{layer_id}` restituisce la cronologia dei suoi anchor. Ogni anchor porta una firma **ML-DSA-87 (Dilithium-5)** sui suoi campi canonici, quindi un client può recuperare un anchor e verificarlo in modo indipendente — la base on-chain per le [ricevute di settlement](/rollups/settlement-receipts) del Rollup Development Kit.
...
| GET    | `/qorechain/multilayer/v1/routing-stats`        | Statistiche di instradamento delle transazioni tra i layer  |
...
| GET    | `/svm/v1/program/{address}` | Informazioni sul program distribuito per un indirizzo di program dato |
...
| GET    | `/inflation/v1/epoch`  | Numero di epoch corrente e relativo avanzamento |
...
| GET    | `/babylon/v1/checkpoint/{epoch}` | Dati del checkpoint BTC per un epoch dato      |
| GET    | `/babylon/v1/params`             | Parametri correnti del modulo Babylon          |

## Modulo Abstract Account

A partire dalla versione di chain **v3.1.85**, il servizio di query abstract-account è esposto via REST tramite grpc-gateway sotto il prefisso `/qorechain/abstractaccount/v1/...`, incluso lo **schema dei permessi** usato per validare gli scope degli [authenticator](/developer-guide/account-abstraction#authenticators) senza hardcoding.

| Metodo | Endpoint                                              | Descrizione                                                     |
| ------ | ----------------------------------------------------- | ---------------------------------------------------------------- |
| GET    | `/qorechain/abstractaccount/v1/config`                | Configurazione del modulo (feature-probe: `enabled`)             |
| GET    | `/qorechain/abstractaccount/v1/accounts`              | Elenca gli abstract account                                      |
| GET    | `/qorechain/abstractaccount/v1/accounts/{address}`    | Stato dell'abstract account per un indirizzo                     |
| GET    | `/qorechain/abstractaccount/v1/permission_schema`     | Tassonomia dei permessi, mappa msg→permesso, msg non delegabili  |
...
| Metodo | Endpoint               | Descrizione                                           |
| ------ | ---------------------- | ----------------------------------------------------- |
| GET    | `/fairblock/v1/config` | Configurazione corrente della crittografia FairBlock  |
...
La riflessione del server gRPC è abilitata per impostazione predefinita, consentendo a strumenti come `grpcurl` di scoprire i servizi disponibili:
...
Tutti gli endpoint REST e gRPC non richiedono autenticazione per impostazione predefinita. Per i deployment in produzione, posiziona un reverse proxy (ad es. Nginx o Caddy) davanti al nodo per gestire la terminazione TLS e il controllo degli accessi.
