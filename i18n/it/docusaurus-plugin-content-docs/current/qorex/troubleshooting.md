...
| **Un account con fondi è scomparso da "I tuoi account"** | Non è andato perso nulla — ogni account viene ri-derivato dalla tua frase di recupero, quindi si tratta di un problema di visualizzazione, non di perdita di dati. Si trattava di un bug reale, ora risolto, presente nelle build mobile precedenti alla **1.0.9**: una lettura momentaneamente fallita dall'archiviazione sicura del dispositivo (molto probabilmente proprio al momento dello sblocco) poteva mostrare un elenco di account ridotto. Aggiorna alla 1.0.9 o successiva; se sei già aggiornato e questo accade, chiudere e riaprire l'app dovrebbe farlo ricomparire — toccare "aggiungi un account" creerebbe un account **nuovo** invece di ripristinare quello mancante, quindi evita di farlo finché l'account esistente non riappare. |
