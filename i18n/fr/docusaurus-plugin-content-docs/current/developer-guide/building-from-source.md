...
La commande `init` devrait créer un fichier de genesis et une configuration de nœud dans `~/.qorechaind/` sans erreurs. L'exemple ci-dessus initialise contre le testnet **`qorechain-diana`** — pour le mainnet, remplacez par `--chain-id qorechain-vladi`, le réseau en service exécutant la version de chaîne **v3.1.77**.
