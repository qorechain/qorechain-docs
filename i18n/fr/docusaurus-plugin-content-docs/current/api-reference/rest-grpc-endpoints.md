...
Tous les points de terminaison REST renvoient du JSON. Les points de terminaison gRPC utilisent les Protocol Buffers et peuvent être consommés avec n'importe quel client gRPC. L'interface RPC fournit des requêtes au niveau du consensus ainsi que la diffusion de transactions.

:::note
Ces interfaces sont disponibles à la fois sur le réseau principal **`qorechain-vladi`** (en service depuis le 7 juin 2026 sur la version de chaîne **v3.1.80**) et sur le réseau de test **`qorechain-diana`**. Les URL de base ci-dessous supposent un nœud exécuté localement ; remplacez-les par l'hôte de réseau principal ou de réseau de test de votre fournisseur pour un accès distant.
:::
...
| GET     | `/ai/v1/fee-estimate`              | Estimation des frais de gaz assistée par AI pour une transaction |
...
À partir de la version de chaîne **v3.1.77**, l'état en lecture seule du module bridge est exposé via REST grâce à grpc-gateway sous le préfixe `/qorechain/bridge/v1/...` (auparavant uniquement gRPC). Ces points de terminaison servent du JSON réel issu de la chaîne via HTTP, à destination des explorateurs et de la télémétrie des nœuds légers. La `config` du bridge indique par exemple `min_validators=10` et `threshold=7`.
...
| GET     | `/qorechain/bridge/v1/chains/{chain_id}`   | Détails d'une chaîne bridge spécifique         |
| GET     | `/qorechain/bridge/v1/validators`          | Liste les validateurs de bridge enregistrés    |
| GET     | `/qorechain/bridge/v1/validators/{address}`| Détails d'un validateur de bridge spécifique   |
| GET     | `/qorechain/bridge/v1/operations`          | Liste les opérations de bridge                 |
| GET     | `/qorechain/bridge/v1/operations/{id}`     | Détails d'une opération de bridge spécifique   |
...
| GET     | `/bridge/v1/chains/{id}`            | Détails d'une chaîne bridge spécifique                |
| GET     | `/bridge/v1/validators`             | Liste les validateurs de bridge actifs                |
| GET     | `/bridge/v1/operations`             | Liste les opérations de bridge récentes               |
| GET     | `/bridge/v1/operations/{id}`        | Détails d'une opération de bridge spécifique          |
| GET     | `/bridge/v1/locked/{chain}/{asset}` | Valeur totale verrouillée pour une paire chaîne/actif |
| GET     | `/bridge/v1/limits/{chain}`         | Limites de débit et seuils pour une chaîne bridge     |
...
| Méthode | Point de terminaison       | Description                                          |
| ------- | -------------------------- | --------------------------------------------------- |
| GET     | `/crossvm/v1/message/{id}` | Récupère un message cross-VM par son ID             |
...
À partir de la version de chaîne **v3.1.80**, le service de requête complet du module multilayer est exposé via REST grâce à grpc-gateway sous le préfixe `/qorechain/multilayer/v1/...` (auparavant uniquement gRPC), y compris deux **requêtes de lecture d'ancrage d'état** : `anchor/{layer_id}` renvoie le dernier ancrage de règlement d'une couche, et `anchors/{layer_id}` renvoie son historique d'ancrages. Chaque ancrage porte une signature **ML-DSA-87 (Dilithium-5)** sur ses champs canoniques, de sorte qu'un client peut récupérer un ancrage et le vérifier de manière indépendante — la base on-chain des [reçus de règlement](/rollups/settlement-receipts) du Rollup Development Kit.
...
| GET     | `/qorechain/multilayer/v1/anchor/{layer_id}`    | Dernier ancrage d'état d'une couche               |
| GET     | `/qorechain/multilayer/v1/anchors/{layer_id}`   | Historique des ancrages d'état d'une couche       |
| GET     | `/qorechain/multilayer/v1/routing-stats`        | Statistiques de routage des transactions entre couches |

Une `StateAnchorView` contient les champs `layer_id`, `layer_height`, `state_root`, `validator_set_hash`, `main_chain_height`, `anchored_at`, `pqc_aggregate_signature`, `transaction_count` et `compressed_state_proof`. Le message canonique signé est `layer_id || layer_height || state_root || validator_set_hash`, vérifié par rapport à la clé PQC enregistrée du créateur de la couche.
...
Les paramètres de réglage de PRISM et l'état de l'agent d'apprentissage par renforcement sont exposés via ce module.
...
À partir de la version de chaîne **v3.1.77**, l'état en lecture seule du module burn est exposé via REST grâce à grpc-gateway sous le préfixe `/qorechain/burn/v1/...` (auparavant uniquement gRPC). Ces points de terminaison servent du JSON réel issu de la chaîne via HTTP, à destination des explorateurs et de la télémétrie des nœuds légers. Les `stats` de burn incluent par exemple `gas_burn_rate=0.30`.
...
| Méthode | Point de terminaison                 | Description                                       |
| ------- | ------------------------------------ | ------------------------------------------------- |
| GET     | `/gasabstraction/v1/accepted-tokens` | Liste les jetons acceptés pour le paiement du gaz |
| GET     | `/gasabstraction/v1/params`          | Paramètres actuels du module Gas Abstraction      |

## Réflexion gRPC

La réflexion de serveur gRPC est activée par défaut, permettant à des outils tels que `grpcurl` de découvrir les services disponibles :
...
Tous les points de terminaison REST et gRPC sont non authentifiés par défaut. Pour les déploiements en production, placez un proxy inverse (par exemple Nginx ou Caddy) devant le nœud afin de gérer la terminaison TLS et le contrôle d'accès.
