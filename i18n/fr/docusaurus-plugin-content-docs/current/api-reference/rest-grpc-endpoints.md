...
Tous les points de terminaison REST renvoient du JSON. Les points de terminaison gRPC utilisent Protocol Buffers et peuvent être consommés avec n'importe quel client gRPC. L'interface RPC fournit des requêtes au niveau du consensus ainsi que la diffusion des transactions.

:::note
Ces interfaces sont disponibles à la fois sur le mainnet **`qorechain-vladi`** (en production depuis le 7 juin 2026 sur la version de chaîne **v3.1.85**) et sur le testnet **`qorechain-diana`**. Les URL de base ci-dessous supposent un nœud exécuté localement ; les points de terminaison publics hébergés (`rpc/api/evm/svm.qore.host` et leurs variantes `-testnet`) sont listés dans [Réseaux](/appendix/networks#public-endpoints).
:::
...
| GET     | `/ai/v1/network/recommendations`   | Recommandations d'optimisation du réseau générées par AI          |
| GET     | `/ai/v1/circuit-breakers`          | États et seuils actuels des disjoncteurs (circuit breakers)       |

## Module Bridge {#bridge-module}

Depuis la version de chaîne **v3.1.77**, l'état en lecture seule du module bridge est exposé en REST via grpc-gateway sous le préfixe `/qorechain/bridge/v1/...` (auparavant gRPC uniquement). Ces points de terminaison servent du JSON on-chain réel via HTTP pour les explorateurs et la télémétrie des nœuds légers. La `config` du bridge indique par exemple `min_validators=10` et `threshold=7`.
...
| GET     | `/qorechain/bridge/v1/operations`           | Liste les opérations du bridge                |
...
| GET     | `/bridge/v1/operations`             | Liste les opérations récentes du bridge               |
...
| GET     | `/svm/v1/program/{address}` | Informations sur un programme déployé pour une adresse de programme donnée  |
...
Depuis la version de chaîne **v3.1.77**, l'état en lecture seule du module burn est exposé en REST via grpc-gateway sous le préfixe `/qorechain/burn/v1/...` (auparavant gRPC uniquement). Ces points de terminaison servent du JSON on-chain réel via HTTP pour les explorateurs et la télémétrie des nœuds légers. Les `stats` de burn incluent par exemple `gas_burn_rate=0.30`.
...
Depuis la version de chaîne **v3.1.85**, le service de requêtes des comptes abstraits est exposé en REST via grpc-gateway sous le préfixe `/qorechain/abstractaccount/v1/...`, y compris le **schéma de permissions** utilisé pour valider les portées des [authentificateurs](/developer-guide/account-abstraction#authenticators) sans codage en dur.
...
| GET     | `/qorechain/abstractaccount/v1/accounts/{address}` | État du compte abstrait pour une adresse                                       |
| GET     | `/qorechain/abstractaccount/v1/permission_schema`  | Taxonomie des permissions, correspondance msg→permission, msgs non délégables  |
...
| Méthode | Point de terminaison                 | Description                                       |
| ------- | ------------------------------------ | ------------------------------------------------- |
| GET     | `/gasabstraction/v1/accepted-tokens` | Liste les jetons acceptés pour le paiement du gas |
| GET     | `/gasabstraction/v1/params`          | Paramètres actuels du module Gas Abstraction      |

## Réflexion gRPC

La réflexion du serveur gRPC est activée par défaut, ce qui permet à des outils tels que `grpcurl` de découvrir les services disponibles :
