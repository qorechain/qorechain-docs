...
| **Un compte financé a disparu de « Vos comptes »** | Rien n'est perdu — chaque compte se redérive de votre phrase de récupération, il s'agit donc d'un problème d'affichage, pas d'une perte de données. C'était un vrai bug, désormais corrigé, sur les versions mobiles antérieures à **1.0.9** : un échec momentané de lecture depuis le stockage sécurisé de l'appareil (le plus probable juste au déverrouillage) pouvait afficher une liste de comptes raccourcie. Mettez à jour vers 1.0.9 ou une version ultérieure ; si vous êtes déjà à jour et que cela se produit, fermer puis rouvrir l'application devrait le faire réapparaître — appuyer sur « ajouter un compte » créerait un **nouveau** compte au lieu de restaurer celui qui manque, donc évitez cela tant que le compte existant n'est pas réapparu. |
