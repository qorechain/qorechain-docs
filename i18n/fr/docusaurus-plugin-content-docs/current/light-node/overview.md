...
- **Vérifiez par recoupement l'état de la chaîne qui vous est présenté.** Le nœud récupère la dernière hauteur depuis son point de terminaison RPC principal et depuis chaque point de terminaison témoin configuré en parallèle, et ne stocke un en-tête que lorsqu'ils s'accordent sur le hachage de bloc — ce qui élève l'exigence : il ne suffit plus de faire confiance à un seul point de terminaison, il faut désormais que tous les points de terminaison configurés soient compromis simultanément. Il s'agit d'une corroboration entre sources indépendantes, **pas d'une vérification cryptographique complète du consensus** (aucune vérification de l'ensemble des validateurs ni des signatures de commit). Votre propre nœud indique le mode dans lequel il fonctionne via un statut `Assurance` sur **son propre tableau de bord** (`http://127.0.0.1:8420` par défaut) — ce n'est pas quelque chose qu'un tableau de bord central QoreChain peut observer, puisque les choix RPC de votre nœud sont locaux à votre configuration. **`trusted-single-source`** (aucun témoin configuré) est la valeur par défaut sur laquelle démarrent la plupart des opérateurs, et non un signal d'alerte — un point de terminaison unique géré honnêtement rapporte la même valeur qu'un point de terminaison compromis. Ajoutez un témoin exploité de manière indépendante pour passer à `corroborated-across-sources`.
